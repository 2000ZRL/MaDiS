...
class BaseLosses(nn.Module):
    ...
    def _update_loss(self, loss: str, outputs, inputs, length=None, dist_func='l1_smooth'):
        '''Update the loss and return the weighted loss.'''
        # print(loss, inputs.shape, outputs.shape)
        # Update the loss
        if loss.split('_')[0] == 'recons':
            val = self._losses_func[loss](outputs, inputs, length=length, dist_func=dist_func)
        else:
